...
from cluster_utils import init_clusters,find_neighbor
...
class MISSRecTrainer(Trainer):
    ...
    def _multimodal_interest_discovery(self, epoch_idx=None):
        self.logger.info(
            f'Discovering multi-modal user interest cluster' + ('' if epoch_idx is None else f' before {epoch_idx}-th epoch'))
        all_embedding_list = []
        self.model.eval()
        ...
        # 导入预训练的cluster
        cluster_idx = find_neighbor(all_embedding, self.centroids)
        # write back to the model
        self.model.interest_embeddings[1:] = self.centroids
        read_ptr = 0
        if 'text' in self.model.modal_type:
            self.model.plm_interest_lookup_table[1: 1 + len(all_text_embeddings)] = cluster_idx[
                                                                                    read_ptr: read_ptr + len(
                                                                                        all_text_embeddings)]
            read_ptr += len(all_text_embeddings)
        if 'img' in self.model.modal_type:
            self.model.img_interest_lookup_table[1: 1 + len(all_img_embeddings)] = cluster_idx[read_ptr: read_ptr + len(
                all_img_embeddings)]
            read_ptr += len(all_img_embeddings)
        assert read_ptr == len(cluster_idx), f"read_ptr={read_ptr}, len(cluster_idx)={len(cluster_idx)}"
        del all_embedding_list, all_embedding, cluster_idx
        self.logger.info(
            f'Finish multi-modal interest cluster discovery' + ('' if epoch_idx is None else f' before {epoch_idx}-th epoch'))
...
class DDPPretrainTrainer(Trainer):
    def __init__(self, config, model):
        ...
        self.centroids=torch.empty(0, 0, dtype=torch.float)
    ...
    def save_pretrained_model(self, epoch, saved_model_file):
        ...
        #保存模型centriods到当前目录 暂用
        torch.save(self.centroids, "centroids.pth")
...
class DDPMISSRecTrainer(DDPTrainer):
    ...
    def _multimodal_interest_discovery(self, epoch_idx=None):
        self.logger.info(
            f'Discovering multi-modal user interest' + ('' if epoch_idx is None else f' before {epoch_idx}-th epoch'))
        ...
        if self.rank == 0:
            ...
            if self.centroids.numel() == 0:
                self.centroids = init_clusters(self.num_clusters, all_embedding.shape[1])  # 返回tensor
            cluster_idx = find_neighbor(all_embedding, self.centroids)
            # all_reduce requires all tensors contigous
            cluster_idx = cluster_idx.contiguous()
            centroids = self.centroids.contiguous()
        ...
        read_ptr = 0
        if 'text' in self.model.modal_type:
            self.model.plm_interest_lookup_table[1: 1 + text_embedding_num] = cluster_idx[
                                                                              read_ptr: read_ptr + text_embedding_num]
            read_ptr += text_embedding_num
        if 'img' in self.model.modal_type:
            self.model.img_interest_lookup_table[1: 1 + img_embedding_num] = cluster_idx[
                                                                             read_ptr: read_ptr + img_embedding_num]
            read_ptr += img_embedding_num
        assert read_ptr == len(cluster_idx), f"read_ptr={read_ptr}, len(cluster_idx)={len(cluster_idx)}"
        del cluster_idx, centroids
        self.logger.info(
            f'Finish multi-modal interest discovery' + ('' if epoch_idx is None else f' before {epoch_idx}-th epoch'))
...
class DDPMISSRecPretrainTrainer(DDPPretrainTrainer):
    ...
    def _multimodal_interest_discovery(self, dataloader, epoch_idx=None, preproc_bs=512):
        ...
        if self.rank == 0:
            ...
            cluster_idx_num = text_embedding_num + img_embedding_num
            cluster_idx = torch.zeros(cluster_idx_num, dtype=torch.long,
        ...
        if 'text' in self.model.modal_type:
            if self.model.train_stage == 'pretrain':
                dataloader.dataset.plm_interest_lookup_table[1: 1 + text_embedding_num] = cluster_idx[
                                                                                          read_ptr: read_ptr + text_embedding_num]
            else:
                dataloader.dataset.plm_interest_lookup_table[1: 1 + text_embedding_num] = cluster_idx[
                                                                                          read_ptr: read_ptr + text_embedding_num]
            read_ptr += text_embedding_num
        if 'img' in self.model.modal_type:
            dataloader.dataset.img_interest_lookup_table[1: 1 + img_embedding_num] = cluster_idx[
                                                                                     read_ptr: read_ptr + img_embedding_num]
            read_ptr += img_embedding_num
        assert read_ptr == len(cluster_idx), f"read_ptr={read_ptr}, len(cluster_idx)={len(cluster_idx)}"
        del cluster_idx, centroids
